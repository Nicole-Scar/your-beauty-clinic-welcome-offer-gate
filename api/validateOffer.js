...
export default async function validateOffer(req, res) {
  try {
    ...
    console.log("🧾 Raw custom fields received:");
    cf.forEach(f => {
      console.log(
        `• Field ID: ${f?.id || "N/A"} | Name: ${f?.name || "N/A"} | Value:`,
        f?.value
      );
    });
    ...
    for (const f of cf) {
      ...
      if (
        process.env.GHL_FIELD_WELCOME_ID &&
        f.id === process.env.GHL_FIELD_WELCOME_ID
      ) {
        welcomeOfferAccess = valueIsYes(f.value);
        console.log(
          "✅ Matched WELCOME field:",
          f.id,
          "→",
          f.value,
          "→",
      ...
      if (
        process.env.GHL_FIELD_OFFERBOOKED_ID &&
        f.id === process.env.GHL_FIELD_OFFERBOOKED_ID
      ) {
        offerBooked = valueIsYes(f.value);
        console.log(
          "✅ Matched OFFER BOOKED field:",
          f.id,
          "→",
          f.value,
          "→",
      ...
      if (f.name && normLower(f.name) === "welcome offer expiry") {
        expiryDate = f.value;
        console.log("📅 Matched EXPIRY field:", f.value);
      }
    }
    ...
    const booleanFields = cf
      ...
      .filter(x =>
        ["yes", "no", "true", "false", "1", "0", ""].includes(x.v)
      );

    if (booleanFields.length === 1) {
      if (welcomeOfferAccess === null)
        welcomeOfferAccess = valueIsYes(booleanFields[0].f.value);
      if (offerBooked === null) offerBooked = false;
    } else if (booleanFields.length >= 2) {
      if (welcomeOfferAccess === null)
        welcomeOfferAccess = valueIsYes(booleanFields[0].f.value);
      if (offerBooked === null)
        offerBooked = valueIsYes(booleanFields[1].f.value);
    }
    ...
    if (!isValid) {
      ...
      console.log("❌ Failure reasons:", {
        missingOptInTag: !hasTag,
        accessNotGranted: !welcomeOfferAccess,
        alreadyBooked: offerBooked,
        expired: isExpired
      });
    }
